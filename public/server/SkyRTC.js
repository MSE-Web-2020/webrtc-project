...
require('util').inherits(SkyRTC, require('events').EventEmitter);
SkyRTC.prototype.addSocket = function (socket) {this.sockets.push(socket)};
...
SkyRTC.prototype.init = function (socket) {
    let that = this;
    socket.id = require('node-uuid').v4();
